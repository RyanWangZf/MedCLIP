...
from medclip.prompts import generate_class_prompts, generate_covid_class_prompts
...
for i in range(5):
    ...
    mode='binary'
    val_data = ZeroShotImageDataset(['covid19-balance'], class_names=constants.COVID_TASKS)
    ...
    clf = GloriaPromptClassifier(gloria_model, ensemble=True)
    evaluator = Evaluator(
        medclip_clf=clf,
    ...
    #     num_workers=0,
    #     )
    # clf = GloriaPromptClassifier(gloria_model, ensemble=False)
    # evaluator = Evaluator(
    #     medclip_clf=clf,
    #     eval_dataloader=eval_dataloader,
    #     mode=mode,
