from data import Biview_OneSent, collate_fn
from wordsat import Encoder2Decoder
...
if __name__ == '__main__':
    ...
    train_set = Biview_OneSent('train', args.dataset_dir, args.train_folds, args.report_path, args.vocab_path, args.label_path)
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=8, collate_fn=collate_fn)
    val_set = Biview_OneSent('val', args.dataset_dir, args.val_folds, args.report_path, args.vocab_path, args.label_path)
    val_loader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=1, collate_fn=collate_fn)
    test_set = Biview_OneSent('test', args.dataset_dir, args.test_folds, args.report_path, args.vocab_path, args.label_path)
    test_loader = DataLoader(test_set, batch_size=1, shuffle=False, num_workers=1, collate_fn=collate_fn)
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        for i, (images1, images2, labels, captions, loss_masks, caseids) in enumerate(train_loader):
            images1 = images1.to(device)
            images2 = images2.to(device)
            captions = captions.to(device)
            loss_masks = loss_masks.to(device)
            optimizer.zero_grad()
            logits = model(images1, images2, captions[:, :-1])
        ...
        if epoch % args.log_freq == 0:
            ...
            model.decoder.eval()
            val_res = {}
            for i, (images1, images2, labels, captions, loss_masks, caseids) in enumerate(val_loader):
                images1 = images1.to(device)
                images2 = images2.to(device)
            ...
            test_res = {}
            for i, (images1, images2, labels, captions, loss_masks, caseids) in enumerate(test_loader):
                images1 = images1.to(device)
                images2 = images2.to(device)
                preds = model(images1, images2)
