...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=1, embed_dim=128, norm_layer=None):
        super().__init__()
...
class Uwinformer(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=1, proj_dim=512,
                 embed_dim=128, depths=[2, 2, 6, 2], num_heads=[4, 8, 8, 16],
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
            x_res = self.avgpool(x.transpose(1,2))
