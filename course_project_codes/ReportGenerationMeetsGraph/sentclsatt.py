...
class SentClsAtt(nn.Module):
    ...
    def __init__(self, num_classes, vocab_size, feat_size=1024, embed_size=256, hidden_size=512):
        ...
        self.word_lstm = nn.LSTMCell(embed_size + hidden_size, hidden_size)
        self.fc = nn.Linear(hidden_size, vocab_size)
        self.dropout = nn.Dropout(0.5)

    def forward(self, images1, images2, captions=None, update_masks=None, stop_id=None, max_sents=10, max_len=30, top=0.2):
        ...
        topk = int(top * h * w)
        cnn_feats1_t = cnn_feats1.permute(0, 2, 3, 1)
        cams1 = self.encoder.densenet121.classifier(cnn_feats1_t)
        cams1 = cams1.permute(0, 3, 1, 2).view(batch_size, self.num_classes, h * w)
        cams1 = cams1 / 16
        cams1 = torch.exp(cams1)
        ths = cams1.topk(topk).values[:, :, -1].unsqueeze(2)
        cams1[cams1 < ths] = 0.0
        cams1 = cams1 / cams1.sum(dim=2, keepdim=True)
        cnn_feats1_t = cnn_feats1_t.view(batch_size, h * w, -1)
        cls_feats1 = torch.bmm(cams1, cnn_feats1_t)

        cnn_feats2_t = cnn_feats2.permute(0, 2, 3, 1)
        cams2 = self.encoder.densenet121.classifier(cnn_feats2_t)
        cams2 = cams2.permute(0, 3, 1, 2).view(batch_size, self.num_classes, h * w)
        cams2 = cams2 / 16
        cams2 = torch.exp(cams2)
        ths = cams2.topk(topk).values[:, :, -1].unsqueeze(2)
        cams2[cams2 < ths] = 0.0
        cams2 = cams2 / cams2.sum(dim=2, keepdim=True)
        cnn_feats2_t = cnn_feats2_t.view(batch_size, h * w, -1)
        cls_feats2 = torch.bmm(cams2, cnn_feats2_t)
        ...
        if captions is not None:
            ...
            for k in range(num_sents):
                context1, alpha1 = self.atten(sent_h, cls_feats1)
                context2, alpha2 = self.atten(sent_h, cls_feats2)
                sent_h, sent_c = self.sent_lstm(torch.cat((context1, context2), dim=1), (sent_h, sent_c))
                seq_len_k = update_masks[:, k].sum(dim=1).max().item()

                for t in range(seq_len_k):
                    batch_mask = update_masks[:, k, t]
                    word_h_, word_c_ = self.word_lstm(torch.cat((embeddings[batch_mask, k, t], sent_h[batch_mask]), dim=1),
            ...
            for k in range(num_sents):
                ...
                for t in range(seq_len):
                    ...
                    word_h, word_c = self.word_lstm(torch.cat((embedding, sent_h), dim=1), (word_h, word_c))
